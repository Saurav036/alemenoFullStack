...
// import { getRefreshToken } from "../redux/auth/authAction";
import { useDispatch } from "react-redux";

const Dashboard = () => {
  const privateReq = useAuth()
  ...
  const getData =async( )=>{
    const res = await privateReq.get('/dashboard/110')
    console.log('dashboard data', res)
    // dispatch( getRefreshToken())
    // console.log('dashboard api ',await dispatch(getRefreshToken()))
        ...
        <div className="flex flex-row justify-center items-start w-full gap-6 ">
          ...
          <div className=" text-violet-600 rounded-2xl p-5 my-10  shadow-md w-full bg-white">
            ...
            <div className="text-sm text-slate-500">
              ...
              technological, social, and environmental advancements that shape
              our future.
            </div>
          </div>
        </div>
